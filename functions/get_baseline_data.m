function [all_full_traces,all_peak_segments,all_ss_segments,...
    mean_peak_segments, min_trace_leng,all_baseline_traces,all_tail_traces] = ...
...
all_tail_traces     = cell(1,numel(all_base_freqs));
...
for ii = 1:numel(all_base_freqs)
    ...
    prot_spec_freq_tail = [base_freq, 0];


    %Extract right traces
    [mean_match_traces,~,mean_match_idx,mean_match_times] = ...
        gather_spec_prot_trace(allData,Fs,...
        prot_spec_freq,prot_spec_dur,washin_state);

    %Extract tail traces
    opts = struct('ignore_first_dur',false);
    [tail_match_traces,~,tail_match_idx,tail_match_times] = ...
        gather_spec_prot_trace(allData,Fs,...
        prot_spec_freq_tail,[prot_spec_dur(end), 2],washin_state,0,opts);
    
    tail_length = round((2 + 1/base_freq) * Fs)-1;
    tail_match_traces = tail_match_traces(:,end-tail_length:end);
    ...
    all_tail_traces{ii}     = tail_match_traces;
end
